...
STORAGE_CONFIG = {
    'KeyIssuer': {
        ...
        'kwargs': {
            'io_class': 'cryptojwt.serialize.item.KeyBundle',
...
def test_build_key_issuer():
    keys = [
        {"type": "RSA", "use": ["enc", "sig"]},
        {"type": "EC", "crv": "P-256", "use": ["sig"]},
    ]

    key_issuer = build_keyissuer(keys, storage_conf=STORAGE_CONFIG)

    assert len(key_issuer) == 3  # A total of 3 keys
...
class TestKeyIssuer(object):
    def test_add_kb(self):
        issuer = KeyIssuer(name='https://issuer.example.com', storage_conf=STORAGE_CONFIG)
        kb = keybundle_from_local_file(RSAKEY, "der", ["ver", "sig"], storage_conf=STORAGE_CONFIG)
        issuer.add_kb(kb)
        assert len(issuer.all_keys()) == 1

    def test_add_symmetric(self):
        issuer = KeyIssuer(storage_conf=STORAGE_CONFIG)
        issuer.add_symmetric('abcdefghijklmnop', ['sig'])
        assert len(issuer.get('oct')) == 1
    ...
    def test_items(self):
        issuer = KeyIssuer(storage_conf=STORAGE_CONFIG)
        issuer.add(KeyBundle(
            [{"kty": "oct", "key": "abcdefghijklmnop", "use": "sig"},
             {"kty": "oct", "key": "ABCDEFGHIJKLMNOP", "use": "enc"}]), storage_conf=STORAGE_CONFIG)
        issuer.add(
            keybundle_from_local_file(RSAKEY, "der", ["ver", "sig"], storage_conf=STORAGE_CONFIG))

        assert len(issuer.items()) == 2

    def test_get_enc(self):
        issuer = KeyIssuer()
        issuer.add(KeyBundle(
            [{"kty": "oct", "key": "a1b2c3d4e5f6g7h8", "use": "sig"},
             {"kty": "oct", "key": "a1b2c3d4e5f6g7h8", "use": "enc"}]), storage_conf=STORAGE_CONFIG)
        issuer.add(
            keybundle_from_local_file(RSAKEY, "der", ["ver", "sig"], storage_conf=STORAGE_CONFIG))
    ...
    def test_dump_issuer_keys(self):
        kb = keybundle_from_local_file("file://%s/jwk.json" % BASE_PATH, "jwks",
                                       ["sig"], storage_conf=STORAGE_CONFIG)
        assert len(kb) == 1
        issuer = KeyIssuer(storage_conf=STORAGE_CONFIG)
        issuer.add(kb)
...
def test_remove_after():
    ...
    _now = time.time()
    for kid in _old:
        key_issuer.mark_as_inactive(kid)

    key_issuer.remove_outdated(_now + 5)
...
def test_copy():
    ...
    assert len(issuer_copy.get('sig', 'oct')) == 0
    assert len(issuer_copy.get('sig', 'rsa')) == 1
...
def test_find():
    ...
    assert issuer.find('https://example.com') == []
