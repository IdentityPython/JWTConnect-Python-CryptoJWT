...
def test_with_2_sym_key():
    ...
    kb = KeyBundle([a, b], storage_conf=STORAGE_CONFIG)
...
def test_rsa_init_under_spec():
    kb = rsa_init(
        {'use': ['enc', 'sig'], 'size': 1024}, storage_conf=STORAGE_CONFIG)
    assert kb
    assert len(kb) == 2
    assert len(kb.get('rsa')) == 2
...
def test_unknown_source():
    with pytest.raises(ImportError):
        KeyBundle(source='foobar', storage_conf=STORAGE_CONFIG)
...
def test_keybundle_from_local_der():
    kb = keybundle_from_local_file(
        "{}".format(RSA0),
        "der", ['enc'], storage_conf=STORAGE_CONFIG)
    assert len(kb) == 1
    keys = kb.get('rsa')
    assert len(keys) == 1
    _key = keys[0]
...
def test_ec_keybundle_from_local_der():
    kb = keybundle_from_local_file(
        "{}".format(EC0),
        "der", ['enc'], keytype='EC', storage_conf=STORAGE_CONFIG)
    assert len(kb) == 1
    keys = kb.get('ec')
    assert len(keys) == 1
    _key = keys[0]
...
def test_creat_jwks_sym():
    a = {"kty": "oct", "key": "highestsupersecret", "use": "sig"}
    kb = KeyBundle([a], storage_conf=STORAGE_CONFIG)
    _jwks = kb.jwks()
...
def test_update_RSA():
    kc = keybundle_from_local_file(RSAKEY, "der", ["sig"], storage_conf=STORAGE_CONFIG)
    assert kc.remote is False
...
def test_dump_jwks():
    a = {"kty": "oct", "key": "highestsupersecret", "use": "sig"}
    b = {"kty": "oct", "key": "highestsupersecret", "use": "enc"}
    kb2 = KeyBundle([a, b], storage_conf=STORAGE_CONFIG)
    ...
    nkb = KeyBundle(source='file://jwks_combo', fileformat='jwks', storage_conf=STORAGE_CONFIG)
...
def test_mark_as_inactive():
    ...
    kb = KeyBundle([desc], storage_conf=STORAGE_CONFIG)
...
def test_httpc_params_1():
    ...
    with responses.RequestsMock() as rsps:
        ...
        kb = KeyBundle(source=source, httpc=requests.request,
                       httpc_params=httpc_params, storage_conf=STORAGE_CONFIG)
        assert kb.do_remote()
...
def test_httpc_params_2():
    ...
    kb = KeyBundle(source='https://login.salesforce.com/id/keys',
                   httpc=requests.request, httpc_params=httpc_params, storage_conf=STORAGE_CONFIG)
...
def test_update_2():
    ...
    kb = KeyBundle(source="file://{}".format(fname), fileformat='jwks', storage_conf=STORAGE_CONFIG)
    assert len(kb) == 1
...
def test_loads_0():
    kb = KeyBundle(JWK0, storage_conf=STORAGE_CONFIG)
    assert len(kb) == 1
    key = kb.get("rsa")[0]
    assert key.kid == 'abc'
    assert key.kty == 'RSA'
...
def test_loads_1():
    ...
    kb = KeyBundle(jwks, storage_conf=STORAGE_CONFIG)
...
def test_dump_jwk():
    kb = KeyBundle(storage_conf=STORAGE_CONFIG)
    ...
    kb2 = KeyBundle(_wk, storage_conf=STORAGE_CONFIG)
...
def test_keys():
    kb = KeyBundle(JWKS_DICT, storage_conf=STORAGE_CONFIG)
...
@pytest.mark.network
def test_jwks_url():
    keys = KeyBundle(source='https://login.salesforce.com/id/keys', storage_conf=STORAGE_CONFIG)
...
def test_key_diff_none():
    _kb = build_key_bundle(key_conf=KEYSPEC, storage_conf=STORAGE_CONFIG)
...
def test_key_bundle_update_3():
    _kb = build_key_bundle(key_conf=KEYSPEC, storage_conf=STORAGE_CONFIG)
    diff = key_diff(_kb, KEYSPEC_5)
    assert set(diff.keys()) == {'add', 'del'}  # Add an EC and delete an RSA key
    update_key_bundle(_kb, diff)
...
def test_key_rollover():
    kb_0 = build_key_bundle(key_conf=KEYSPEC, storage_conf=STORAGE_CONFIG)
...
def test_build_key_bundle_sym():
    _kb = build_key_bundle(key_conf=KEYSPEC_6, storage_conf=STORAGE_CONFIG)
    assert len(_kb) == 3
...
def test_key_bundle_difference_none():
    _kb0 = build_key_bundle(key_conf=KEYSPEC_6, storage_conf=STORAGE_CONFIG)
    _kb1 = KeyBundle(storage_conf=STORAGE_CONFIG)
...
def test_key_bundle_difference():
    _kb0 = build_key_bundle(key_conf=KEYSPEC_6, storage_conf=STORAGE_CONFIG)
    _kb1 = build_key_bundle(key_conf=KEYSPEC_2, storage_conf=STORAGE_CONFIG)
...
def test_unique_keys_1():
    _kb0 = build_key_bundle(key_conf=KEYSPEC_6, storage_conf=STORAGE_CONFIG)
    _kb1 = build_key_bundle(key_conf=KEYSPEC_6, storage_conf=STORAGE_CONFIG)
...
def test_init_key():
    ...
    filename = full_path("../tests/tmp_jwk.json")
...
def test_export_inactive():
    ...
    kb2 = KeyBundle(storage_conf=STORAGE_CONFIG).load(res)
...
def test_remote():
    ...
    exp = kb.dump()
    kb2 = KeyBundle(storage_conf=STORAGE_CONFIG).load(exp)
