...
def test_jws_rsa_verifier_with_rfc():
    ...
    for _token in modify_token(
        test_vector.rsa_token, ["RS384", "RS512", "PS256", "PS384", "PS512"]
    ):
...
def test_jws_verifier_with_kid():
    ...
    # Verify
    pub_key = key_from_jwk_dict(json.loads(test_vector.test_json_ecdsa_pub_key_kid1))
...
def test_jws_mac_verifier_with_rfc():
    ...
    for modified_token in modify_token(test_vector.hmac_token, ["HS384", "HS512"]):
        with pytest.raises(JWKESTException):
...
def test_jws_mac_authenticator_and_verifier():
    ...
    for alg in algs:
        ...
        authenticator = JWS(test_vector.test_payload, **json.loads(json_header_hmac))
